...
serve(async (req) => {
  ...
  try {
    // Request body size limit (500KB for image data)
    const bodyText = await req.text();
    if (bodyText.length > 512000) {
      return new Response(
    ...
    const { imageData, mimeType, clinicalNotes, previousAssessment, patientInfo } = JSON.parse(bodyText);
