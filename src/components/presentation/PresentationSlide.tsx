...
export const PRESENTATION_SLIDES: SlideConfig[] = [
  ...
  },
  {
    ...
    talkingPoints: [
      ...
      'Medications: particularly those known to increase fall risk, including sedatives, opioids, antihypertensives, certain psychotropics.',
  ...
  },
  {
    ...
    talkingPoints: [
      ...
      'Starting on the left: the hospital EHR, including Epic, Cerner, Meditech, or any compliant system.',
  ...
  },
  {
    ...
    talkingPoints: [
      ...
      'At the top: aggregate statistics, including patient count, high-risk patients by category, upcoming assessment due times.',
  ...
  },
  {
    ...
    talkingPoints: [
      ...
      'I am not there yet.',
      'The provisional patent covers the technical approach, specifically the architecture, SHAP integration, adaptive thresholds. It does not claim validated clinical outcomes.',
  ...
  },
  {
    ...
    icon: <Award className="w-8 h-8" />,
    notes: [
      'Multi-site validation across 172 hospitals',
      'IDI: 65,157 patients · DBS: 28,362 patients',
      'DBS System: AUROC 0.802 internal, 0.758 external',
      'Target: 87% alert reduction (simulated, not clinically validated)',
    ],
    keyPoints: [
      '28,362 patients validated across internal and external datasets',
      '172 hospitals in external validation (eICU)',
      'DBS system validated with strong discrimination and safety metrics',
      'Target alert reduction: 87% (simulated, not clinically validated)',
      'Target equity disparity: <0.5% across demographics (design goal)',
    ],
    talkingPoints: [
      'Now let me share the actual research validation data from my doctoral research.',
      'The Documentation Burden Score system was validated on 28,362 patients across internal and external datasets, including 172 hospitals.',
      'Both internal and external validation demonstrated strong discrimination and safety metrics.',
      'Separately, the Trust-Based Alert System targets an 87% reduction in non-actionable alerts — this is a simulated design target, not yet clinically validated.',
      'The goal is that nurses would see far fewer distracting alerts, while maintaining sensitivity for critical events.',
      'Our equity monitoring design target is less than 0.5% disparity across demographic groups — again, a goal requiring clinical validation.',
      'The DBS validation results form the foundation for the ANIA 2026 presentation in Boston.',
    ],
  ...
  },
  {
    ...
    notes: [
      ...
      'Addresses alert fatigue - target 87% reduction (simulated)',
      'Dynamic thresholds based on nurse workload',
      'Design target: maintain sensitivity for critical alerts',
    ],
    keyPoints: [
      'Target: 87% reduction in non-actionable alerts (simulated)',
      'Design goal: maintain sensitivity for critical events',
      'Dynamic thresholds adjust to nurse workload',
      'Trust Score based on historical precision',
      'Target: 2.3 minutes saved per decision (simulated)',
    ],
    talkingPoints: [
      ...
      'The simulated result: an 87% reduction in alert volume while targeting full sensitivity for true critical events. This is a design goal, not yet validated in clinical settings.',
      'The target is saving an average of 2.3 minutes per clinical decision — time that would go back to patient care. This requires prospective validation.',
  ...
  },
  {
    ...
    keyPoints: [
      ...
      'Multiple clinical acuity and complexity factors',
  ...
  },
  {
    ...
    keyPoints: [
      'Four weighted trust components',
    ...
    talkingPoints: [
      ...
      'It combines four weighted factors — historical accuracy, clinician feedback, temporal relevance, and data quality — using a proprietary weighting scheme.',
  ...
  },
  {
    ...
    icon: <TrendingUp className="w-8 h-8" />,
    notes: [
      'Design-phase performance targets (not yet validated)',
      '87% alert reduction (simulated target)',
      'Requires prospective clinical validation',
    ],
    keyPoints: [
      'Baseline EHR vs ChartMinder comparison (design targets)',
      '87% reduction in non-actionable alerts (simulated)',
      'All metrics are design targets requiring clinical validation',
    ],
    talkingPoints: [
      'This performance comparison shows our design targets for ChartMinder against baseline EHR systems.',
      'Our target is an 87% reduction in non-actionable alerts — this is simulated, not yet clinically validated.',
      'All metrics shown are design goals. Prospective validation is required before any performance claims can be made.',
    ],
    transitionCue: '→ "Now let me show the research validation visualizations..."',
  },
  {
    ...
    keyPoints: [
      ...
      'DBS vs LOS: Correlation analysis (details under NDA)',
      'Quartile Distribution: Balanced Q1-Q4',
      'Staffing Impact: Projected improvements (design targets)',
    ],
    talkingPoints: [
      'These visualizations come directly from the patent documentation.',
      'The Feature Importance chart shows acuity and organ dysfunction scores as top predictors - this aligns with clinical intuition.',
      'The ROC curve demonstrates strong discriminative ability: AUROC 0.802 internal, 0.758 external.',
      'The DBS vs Length of Stay scatter plot shows a meaningful correlation between documentation burden and patient complexity.',
      'The quartile distribution shows balanced patient stratification - not skewed toward any single category.',
      'Staffing impact projections are design targets requiring prospective validation.',
    ],
