...
const priorityStyles = {
  urgent: { border: 'border-l-red-500', badge: 'bg-red-100 text-red-800' },
  high: { border: 'border-l-amber-500', badge: 'bg-amber-100 text-amber-800' },
  routine: { border: 'border-l-emerald-500', badge: 'bg-emerald-100 text-emerald-800' },
};
  ...
  return (
    <div>
      ...
      <div className="space-y-3">
        {tasks.map((t) => (
          <div key={t.id} className={`border-l-4 ${priorityStyles[t.priority].border} bg-muted/30 rounded-lg p-4 flex items-start gap-3 hover:bg-muted/50 transition-colors`}>
            <Checkbox
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className={`text-[11px] font-semibold px-2 py-0.5 rounded uppercase ${priorityStyles[t.priority].badge}`}>{t.priority}</span>
              </div>
    ...
    <div className="space-y-6">
      <Card>
        ...
        <CardContent className="space-y-8">
          <TaskGroup label="Urgent (Complete in next 30 min)" color="text-red-600" tasks={urgentTasks} />
          <TaskGroup label="High Priority (Complete before end of shift)" color="text-amber-600" tasks={highTasks} />
          <TaskGroup label="Routine (Batch when possible)" color="text-emerald-600" tasks={routineTasks} />
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <div className="border-l-4 border-l-blue-500 bg-blue-50 p-4 rounded-lg">
            ...
            <p className="text-sm text-muted-foreground">3 high-priority tasks in East Wing (Rooms 08-A, 10-B, 12-A). Suggested route: 08-A &rarr; 10-B &rarr; 12-A to minimize walking time.</p>
          </div>
        </CardContent>
      </Card>
    </div>
