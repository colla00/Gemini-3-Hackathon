    ...
    >
      ...
      <div className="absolute inset-0 flex flex-wrap items-center justify-center gap-y-32">
        {Array.from({ length: rows * cols }).map((_, index) => (
          <div
            key={index}
            className="flex items-center gap-2 text-foreground/[0.07] dark:text-foreground/[0.03] -rotate-[25deg] whitespace-nowrap"
            style={{
              width: `${100 / cols}%`,
      ...
      <div className="absolute bottom-16 right-4 flex items-center gap-1.5 text-foreground/[0.1] dark:text-foreground/[0.05]">
