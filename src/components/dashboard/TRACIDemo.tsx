...
import { Activity, Clock, TrendingUp, AlertTriangle, Zap, Timer, Shield, DollarSign, ArrowRight } from 'lucide-react';
...
const horizons = [
  { label: '4-Hour', key: '4h', baseRisk: 0.23, trend: 'stable' },
  { label: '12-Hour', key: '12h', baseRisk: 0.41, trend: 'rising' },
  { label: '24-Hour', key: '24h', baseRisk: 0.58, trend: 'rising' },
  { label: '48-Hour', key: '48h', baseRisk: 0.72, trend: 'critical' },
];

const generateTimeline = () => Array.from({ length: 48 }, (_, i) => ({
  hour: i,
...
const patients = [
  { id: 'PT-4821', name: 'R. Martinez', age: 67, risk4h: 0.18, risk48h: 0.71, trajectory: 'escalating', phenotype: 'Escalating Crisis', alertsGenerated: 3 },
  { id: 'PT-3199', name: 'J. Thompson', age: 74, risk4h: 0.62, risk48h: 0.85, trajectory: 'critical', phenotype: 'Chaotic Instability', alertsGenerated: 7 },
  { id: 'PT-5502', name: 'M. Chen', age: 55, risk4h: 0.08, risk48h: 0.12, trajectory: 'stable', phenotype: 'Steady Surveillance', alertsGenerated: 0 },
  { id: 'PT-2847', name: 'A. Williams', age: 81, risk4h: 0.34, risk48h: 0.56, trajectory: 'rising', phenotype: 'Minimal Documentation', alertsGenerated: 2 },
];
...
export const TRACIDemo = () => {
  ...
  // Animate risk gauges in
  useEffect(() => {
    horizons.forEach((h, i) => {
      setTimeout(() => setLiveRisks(prev => ({ ...prev, [h.key]: h.baseRisk })), 200 + i * 150);
    });
  }, [selectedPatient]);
      ...
      setLiveRisks(prev => {
        ...
        horizons.forEach(h => {
          if (updated[h.key] !== undefined) {
            updated[h.key] = Math.min(0.99, Math.max(0.05, updated[h.key] + (Math.random() - 0.48) * 0.02));
          }
        });
        return updated;
      });
        ...
        {horizons.map((h, i) => {
          ...
          return (
            <motion.div key={h.key} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 + i * 0.1 }}>
              <Card className={cn('border-border/40 transition-all', risk > 0.6 && 'border-destructive/30 shadow-sm shadow-destructive/5')}>
                <CardContent className="p-4 text-center">
